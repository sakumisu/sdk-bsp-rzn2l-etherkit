...
static rt_err_t ra_pin_irq_enable(struct rt_device *device, rt_base_t pin, rt_uint8_t enabled)
{
    ...
    if (PIN_IRQ_ENABLE == enabled)
    {
        ...
        {
            err = R_ICU_ExternalIrqOpen((external_irq_ctrl_t *)pin_irq_map[irqx].irq_ctrl,
                                        (external_irq_cfg_t const *)pin_irq_map[irqx].irq_cfg);
            /* Handle error */
            if (FSP_SUCCESS != err)
            {
            ...
            err = R_ICU_ExternalIrqEnable((external_irq_ctrl_t *)pin_irq_map[irqx].irq_ctrl);
            /* Handle error */
            if (FSP_SUCCESS != err)
            {
    ...
    {
        err = R_ICU_ExternalIrqDisable((external_irq_ctrl_t *)pin_irq_map[irqx].irq_ctrl);
        if (FSP_SUCCESS != err)
        {
        ...
        err = R_ICU_ExternalIrqClose((external_irq_ctrl_t *)pin_irq_map[irqx].irq_ctrl);
        if (FSP_SUCCESS != err)
        {
