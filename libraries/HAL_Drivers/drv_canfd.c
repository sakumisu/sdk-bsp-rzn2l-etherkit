...
#ifdef BSP_USING_CANFD0
    can_obj[CANFD0_INDEX].can_dev.config = config;
    can_obj[CANFD0_INDEX].can_dev.config.msgboxsz = 64;
    can_obj[CANFD0_INDEX].can_dev.config.sndboxnumber = 1;
    can_obj[CANFD0_INDEX].can_dev.config.ticks = 50;
#endif
#ifdef BSP_USING_CANFD1
    can_obj[CANFD1_INDEX].can_dev.config = config;
    can_obj[CANFD1_INDEX].can_dev.config.msgboxsz = 64;
    can_obj[CANFD1_INDEX].can_dev.config.sndboxnumber = 1;
    can_obj[CANFD1_INDEX].can_dev.config.ticks = 50;
#endif
}
rt_err_t ra_can_configure(struct rt_can_device *can_dev, struct can_configure *cfg)
{
    ...
    err = R_CANFD_Open(can->config->p_api_ctrl, can->config->p_cfg);
    if (FSP_SUCCESS != err)
    {
        return -RT_ERROR;
    }
...
rt_err_t ra_can_control(struct rt_can_device *can_dev, int cmd, void *arg)
{
    ...
    case RT_DEVICE_CTRL_CLR_INT:
        R_BSP_IrqClearPending((IRQn_Type)(uintptr_t)arg);
        break;
    ...
    case RT_CAN_CMD_SET_MODE:
        ...
        if (argval != can->can_dev.config.mode)
        {
            ...
            R_CANFD_ModeTransition(can->config->p_api_ctrl, ((canfd_instance_ctrl_t *)(can->config->p_api_ctrl))->operation_mode, mode_to_set);
        }
        break;
    case RT_CAN_CMD_GET_STATUS:
        R_CANFD_InfoGet(can->config->p_api_ctrl, &can_info);
...
rt_ssize_t ra_can_sendmsg(struct rt_can_device *can_dev, const void *buf, rt_uint32_t boxno)
{
    ...
    if (R_CANFD_Write(can->config->p_api_ctrl, boxno, &g_can_tx_frame) != FSP_SUCCESS)
    {
        rt_exit_critical();
        return -RT_ERROR;
    }
...
void canfd0_callback(can_callback_args_t *p_args)
{
    ...
    case CAN_EVENT_TX_COMPLETE:
        rt_hw_can_isr(&can_obj[CANFD0_INDEX].can_dev, RT_CAN_EVENT_TX_DONE | p_args->mailbox << 8);
        break;
    case CAN_EVENT_RX_COMPLETE:
        can_obj[CANFD0_INDEX].callback_args = p_args;
        if (p_args->event == CAN_EVENT_RX_COMPLETE)
            rt_hw_can_isr(&can_obj[CANFD0_INDEX].can_dev, RT_CAN_EVENT_RX_IND | p_args->mailbox << 8);
        break;
    case CAN_EVENT_TX_ABORTED:
        rt_hw_can_isr(&can_obj[CANFD0_INDEX].can_dev, RT_CAN_EVENT_TX_FAIL | p_args->mailbox << 8);
        break;
...
void canfd1_callback(can_callback_args_t *p_args)
{
    ...
    {
    case CAN_EVENT_TX_COMPLETE:
        rt_hw_can_isr(&can_obj[CANFD1_INDEX].can_dev, RT_CAN_EVENT_TX_DONE | p_args->mailbox << 8);
        break;
    case CAN_EVENT_RX_COMPLETE:
        can_obj[CANFD1_INDEX].callback_args = p_args;
        if (p_args->event == CAN_EVENT_RX_COMPLETE)
            rt_hw_can_isr(&can_obj[CANFD1_INDEX].can_dev, RT_CAN_EVENT_RX_IND | p_args->mailbox << 8);
        break;
    case CAN_EVENT_TX_ABORTED:
        rt_hw_can_isr(&can_obj[CANFD1_INDEX].can_dev, RT_CAN_EVENT_TX_FAIL | p_args->mailbox << 8);
        break;
    case CAN_EVENT_MAILBOX_MESSAGE_LOST:    //overwrite/overrun error event
    case CAN_EVENT_BUS_RECOVERY:            //Bus recovery error event
    case CAN_EVENT_ERR_BUS_OFF:             //error Bus Off event
